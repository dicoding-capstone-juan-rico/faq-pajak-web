        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-4">
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="h-14 px-8 bg-[#022c22] dark:bg-[#cdfc4d] text-white dark:text-[#022c22] rounded-full font-semibold text-lg flex items-center gap-3 shadow-xl hover:shadow-2xl hover:shadow-[#022c22]/20 transition-shadow"
            >
              Start Asking
...
const FAQ = () => {
    return (
        <section className="py-24 bg-white dark:bg-[#021f18] transition-colors duration-500">
             <div className="max-w-4xl mx-auto px-6 text-center">
                <h2 className="text-4xl font-bold text-[#022c22] dark:text-white mb-12">Simplicity in every step</h2>
                <div className="flex flex-col md:flex-row justify-between items-center gap-8 relative">
                    {/* Connector Line */}
                    <div className="hidden md:block absolute top-12 left-0 w-full h-0.5 bg-gray-100 dark:bg-white/10 -z-10" />

                    {[
                        { title: "Ask naturally", icon: <MessageSquare />, desc: "Use everyday language." },
                        { title: "AI Analyzes", icon: <Sparkles />, desc: "Scans 10,000+ pages." },
                        { title: "Get Answer", icon: <CheckCircle2 />, desc: "Clear, cited, actionable." }
                    ].map((step, idx) => (
                        <div key={idx} className="bg-white dark:bg-[#0a271f] p-8 rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-lg w-full md:w-1/3">
                            <div className="w-16 h-16 mx-auto bg-[#f0fdf4] dark:bg-[#cdfc4d]/10 text-[#166534] dark:text-[#cdfc4d] rounded-2xl flex items-center justify-center mb-6 text-xl">
                                {step.icon}
                            </div>
                            <h3 className="text-lg font-bold text-[#022c22] dark:text-white mb-2">{step.title}</h3>
...
const App = () => {
    ...
    return (
        <div className={`min-h-screen transition-colors duration-500 font-sans ${isDark ? 'dark bg-[#022c22]' : 'bg-white'}`}>
             {/* Font Import in a real app would be in HTML head, but here we assume generic sans fits or use style injection */}
             <style>{`
